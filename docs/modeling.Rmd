...
원래 순위 예측과는 다르게 Classification이 아닌 Regression으로 순위를 예측하려고 한다. Regression을 통해 실제 순위와 얼마나 차이 나는지 파악했다. 그리고 Classification의 경우 정확하게 예측하지 않는 이상 예측 평가에서 매우 떨어지기 때문에 그것보다는 최대한 가깝게 맞출 수 있는 Regression을 선택했다.
...
왜냐하면 11위와 19위는 똑같은 범주 안에 묶여있고 주 단위로는 크게 변하지 않기 때문이다.  
...
- 10위 단위가 아닌 실제 순위와 얼마나 가깝게 예측하는 지 확인하다. 
...
- 대부분의 모델에서는 Previous ranking이 가장 중요하게 나왔지만, XGboost에서는 나름 중요한 변수이긴 했으나 6개 중에서는 나오지 않았다. 
- 모델의 성능 평가를 보면 다른 모델에 비해 높은 성능을 보여주고 있다. 
